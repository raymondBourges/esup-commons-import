...
public class EmailExceptionServiceImpl extends SimpleExceptionServiceImpl {
	...
	private String html(final Collection<String> strings) {
		...
		for (String string : strings) {
			result.append(separator).append(StringUtilsWeb.escapeHtml(string));
			separator = "<br>";
		}
	...
	private StringBuffer getHtmlReportException() {
		...
		if (t != null) {
			...
			name = StringUtilsWeb.escapeHtml(cause.getClass().getSimpleName());
			message = StringUtilsWeb.escapeHtml(cause.getMessage());
